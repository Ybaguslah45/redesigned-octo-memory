...
with st.form("entry_form"):
    ...
    with col1:
        usd_value = st.text_input("Nilai (USD):", value=st.session_state.usd_value, placeholder="Masukkan nilai...", key="usd_input")
    with col2:
    ...
    if submitted:
        try:
            ...
            new_data = pd.DataFrame([{"Tanggal": datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "USD": usd_val, "Catatan": catatan}])
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=pd.to_datetime(df["Tanggal"]), y=df["USD"], mode='lines+markers', name='Mood USD'))
...
if not df.empty:
    for i in reversed(df.index):
        row = df.loc[i]
        with st.expander(f"{row['Tanggal']} - ${row['USD']:.2f}"):
